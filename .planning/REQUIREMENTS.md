...
- [x] **PARSE-01**: Engine parses YAML frontmatter from .flow.md files (name, version, trigger, config, secrets, input/output schemas)
- [x] **PARSE-02**: Engine parses XML body into node AST (source, transform, sink, control flow nodes)
- [x] **PARSE-03**: Engine preserves source locations for error messages
- [x] **PARSE-04**: Engine handles malformed input with clear error messages
- [x] **PARSE-05**: Parser is secure against XXE injection (DTD disabled)
- [x] **PARSE-06**: Parser is secure against YAML deserialization attacks (safe mode)
