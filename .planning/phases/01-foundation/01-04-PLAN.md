...
must_haves:
  truths:
    ...
    - "Pattern {{$secrets.NAME}} correctly extracts NAME"
  artifacts:
    - src/validator/references.ts
    - src/validator/references.test.ts
  key_links:
...
<task type="auto">
  ...
  <files>src/validator/references.ts</files>
  <action>
Modify the validateReferences function signature and implementation:

1. Accept WorkflowAST (already does) - has sourceMap.source
2. Pass ast.sourceMap.source to validateSecretReferences
3. Create a new approach: scan the entire source for secret patterns, not just node configs
...
Find all matches of `{{$secrets.NAME}}` in source, extract NAME, check against declaredSecrets.
For location, find the match position in source and convert to line/column.
  </action>
  <verify>bun run typecheck</verify>
  <done>validateReferences passes source to secret validation</done>
</task>
