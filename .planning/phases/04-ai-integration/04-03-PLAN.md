...
must_haves:
  truths:
    - AI runtime calls OpenRouter via direct fetch with tool-based structured output
    - System and user prompts extracted from node.config (populated by parser extractChildElements)
  ...
  key_links:
    ...
    - runtime uses fetch to OpenRouter API (no external SDK needed)
    - prompts accessed via node.config.system and node.config.user (string values from parser)
---

<objective>
Create the AI runtime that calls OpenRouter via fetch with structured output validation.
...
<!-- Parser output format - shows how child elements become config -->
@src/parser/body.ts (extractChildElements function)
...
<task type="auto">
  <name>Task 1: Create AI runtime implementation</name>
  <files>src/runtimes/ai/runtime.ts</files>
  <action>
Create `src/runtimes/ai/runtime.ts` implementing the AI runtime.

**IMPORTANT:** The parser's `extractChildElements` function stores child element content in `node.config`. For an AI node like:
```xml
...
node.config = {
  ...
  system: "You are a sentiment analyzer.",  // from <system> child
  user: "Analyze: {{input.text}}"           // from <user> child
}
```

So prompts are accessed via `config.system` and `config.user`, NOT via `node.children`.
...
/**
 * AI Runtime configuration from node attributes and child elements.
 *
 * The parser extracts:
 * - Attributes: model, output-schema, max-tokens, max-retries, timeout
 * - Child elements: system (prompt), user (prompt)
 */
interface AIRuntimeConfig {
  /** OpenRouter model ID (e.g., "anthropic/claude-3.5-sonnet") */
  model: string;
  /** System prompt from <system> child element */
  system?: string;
  /** User prompt from <user> child element */
  user?: string;
...
class AIRuntime implements NodeRuntime<AIRuntimeConfig, unknown, unknown> {
  ...
  async execute(params: ExecutionParams<AIRuntimeConfig, unknown>): Promise<unknown> {
    ...
    // Extract prompts from config (populated by parser from child elements)
    // Then resolve template expressions
    const systemPromptRaw = config.system;
    const userPromptRaw = config.user;

    const systemPrompt = systemPromptRaw
      ? evaluateTemplateInContext(systemPromptRaw, state)
      : undefined;
    const userPrompt = userPromptRaw
      ? evaluateTemplateInContext(userPromptRaw, state)
      : undefined;
...
<task type="auto">
  <name>Task 2: Create AI runtime barrel export and register</name>
...
- [ ] aiRuntime.type === 'ai'
- [ ] Runtime extracts system/user prompts from node.config (not node.children)
