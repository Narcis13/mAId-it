...
must_haves:
  ...
  artifacts:
    - src/execution/executor.ts (execute, executeWave, executeNode functions)
  ...
  key_links:
    - executor.ts uses Semaphore from scheduler/index.ts (re-exported from concurrency.ts)
    - executor.ts uses ExecutionPlan from scheduler/types.ts
    - executor.ts calls runtime.execute via registry.getRuntime
    - executor.ts uses evaluateTemplateInContext from execution/index.ts (NOT resolveTemplateValue)
---
...
**IMPORTANT:** The executor signature passes `nodes: Map<string, NodeAST>` through the call chain
so that control flow handlers (parallel, foreach) can access body nodes. This is required for
plans 06-03 and 06-04.
...
```
  ...
  <done>Executor compiles with executeNode accepting nodes map parameter</done>
</task>
...
# Verify imports work
bun -e "import { execute, executeNode } from './src/execution/executor'; console.log('Executor OK')"
```
</verification>
...
- [ ] executeNode accepts nodes map parameter for control flow handlers
- [ ] Executor uses Semaphore from scheduler/index.ts
- [ ] Executor uses evaluateTemplateInContext (NOT resolveTemplateValue)
