...
must_haves:
  ...
  key_links:
    ...
    - ForeachResult uses bodyNodeIds (string[]) - executor looks up actual NodeAST from nodes map
---
...
<task type="auto">
  ...
  <action>
Read the existing executor.ts (modified in 06-03) and add support for parallel foreach iterations.

**IMPORTANT CLARIFICATION:**
- `ForeachResult.bodyNodeIds` is `string[]` (array of node IDs, not NodeAST objects)
- The executor must look up actual NodeAST from the `nodes` map using these IDs
- Body nodes are regular nodes (source, transform, sink) that won't return ForeachResult
- This means executeNode called on body nodes won't trigger recursion back to handleForeachResult
- The foreach runtime extracts IDs from ForeachNode.body (which is NodeAST[])
...
After the ParallelResult check (added in 06-03), add:
...
if (isForeachResult(output)) {
  const foreachOutputs = await handleForeachResult(
    output,
    nodes,  // nodes map available from executeNode parameters
    state,
    maxConcurrency
  );
...
- [ ] ForeachResult.bodyNodeIds lookup from nodes map works correctly
