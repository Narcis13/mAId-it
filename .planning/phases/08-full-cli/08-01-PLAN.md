...
must_haves:
  truths:
    ...
    - "Workflow executes all nodes in correct wave order"
    - "Progress spinner shows current wave during execution (e.g., 'Executing wave 1/3...')"
    - "Execution results collected and returned after all waves complete"
...
4. **Progress display with wave-level updates:**

The executor runs waves atomically (all nodes in a wave execute before moving to next wave). Use ora spinner with wave-level progress updates:

```typescript
import ora from 'ora';

// Before execution, get wave count from execution plan
const totalWaves = executionPlan.waves.length;
const spinner = ora('Preparing execution...').start();

// The executor's execute() function runs waves sequentially.
// To show wave progress, execute waves manually:
for (let waveIndex = 0; waveIndex < totalWaves; waveIndex++) {
  spinner.text = `Executing wave ${waveIndex + 1}/${totalWaves}...`;

  // Execute single wave using executeWave or equivalent
  // If executor only exposes execute(), track progress via state changes
  await executeWave(state, executionPlan.waves[waveIndex]);
}

// After all waves complete
spinner.succeed(`Completed ${totalWaves} waves in ${duration}ms`);
```

**Alternative if executeWave not exposed:** Check if executor emits events or allows callbacks. If execute() is the only entry point and runs all waves atomically, show simpler progress:
```typescript
spinner.text = `Executing ${totalWaves} waves...`;
await execute(state, executionPlan);
spinner.succeed(`Completed in ${duration}ms`);
```

Choose the approach that matches the executor's actual API surface.
...
- On failure: spinner.fail() with error message
  ...
  <done>run.ts exports runWorkflow function with proper types, wave-level progress display, and implementation</done>
</task>
...
- [ ] Progress spinner shows wave-level progress during execution
